...
First there was Base R created on August the late 90s, but with many more years of baggage from the S era.   
...
It's vital that you understand that any speed analysis among packages on the individual personal computer level gets turned irrelevant as long as you get access to Spark Clusters, this is how you can query billions of records with ease, not by having slightly faster performance on a single machine level but by leveraging the horizontal scaling capacity of big companies such as Amazon, Google and Microsoft.

# Problems with pandas

I classify two kinds of problems on the pandas framework, the first and honestly the simplest to explain are things that it implements and I think it shouldn't a lot were copied from base R and that is why it was important that you understood the timeline of the packages at the beginning of this post, and the second are the new features that recent packages on both the R and Python sphere put into the table in recent years, features that when pandas was being created didn't exist and that I hope the pandas or some other packages teams will eventually be able to integrate into the pandas ecosystem.

## What pandas has and it shouldn't

Here are the things that I hope that pandas makes easier to get away from or ditches altogether.


### Indexing

This wasn't even a feature back in the sql days, indexing comes from languages that tried to facilitate the well Index of data, so you could play a version of Battleship on your data, the idea sounds good, and it works for mostly simply indexes, data[x,y] if you can picture it, and even the index of rows what R call row.names is in my opinion a mistake, it adds unecessary flutter to what could have been easily one more column, so you can define all datasets or at least all structured datasets ass the tidy diagram shows

![hadley tidy diagram](https://www.openscapes.org/img/blog/tidydata/tidydata_1.jpg)

The id column on this example is treated as just one more column, it doesn't need any special treatment as we will discuss on Slicing.

#### MultiIndex

This is the worst of the bunch, if data[x,y] is discussable and maybe human readable, or excel friendly as I like to call it, MultiIndex is the epitome of unnecessary structure, it burden's you with a new more complex data type than the one that you started, it is hardly an easy feature on a usability standpoint as multiple SO threads will show, it causes confusion, generates code to get back into a flatter structure, it only exists on pandas and copycats, and for good reason, this format is impossible to translate into our common file structures such as csv or xlsx, it can't be passed back into a sql server or Spark, it is too much for what could have been a flatter structure with more names, or a nested return dataframe.


#### The Solution

There are two types of functions that suffer on a practical/usubility standpoint because of the Index concept


The first are the functions that just don't work out of the box because of Indexing, I am talking about not even talking about functions that have bad names or arguments because of indexing like the sort_values, not sort because god forbid sort messed up with the index and you didn't realize it, the biggest issue comes from functions that almost always come accopanied by reset_index() or have some internal argument to ignore_index like concat.
...
These were a low target slicing suffers from the sins of indexing but it is chaos incarnate here are 4 different valid approaches to the same thing



, but this is a superfulos feature that can totally circunvented by two kinds of operations you **select** to get you y and you **where/filter/query** to get x that is

## What pandas hasn't and it should


This example is also useful to prove my point on how hard it is to extend pandas on a user level, you would need to inherit the object in order to create your own pandas method to solve the multi index issue, there should be an easier way to just pass a function that creates a pandas method something like df.apply_method(my_new_function) so that you don't need to break the nice chaning of operations, pipe attempts to do this but it doesn't always works as expected
